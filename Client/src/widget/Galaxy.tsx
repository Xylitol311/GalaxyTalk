  ...
  const { positions, attributes } = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const randomOffset = new THREE.Vector3(
        ...
        (Math.random() - 0.5) * 2,
        (Math.random() - 0.5) * 2
      ).normalize().multiplyScalar(radiusRatio * 0.2 + 0.1) // 오프셋 범위 축소
        ...
        <points>
          <bufferGeometry>
            <bufferAttribute attach="attributes-position" 
              args={[positions, 3]}
              count={count} />
            <bufferAttribute attach="attributes-aRadiusRatio" 
              args={[attributes.aRadiusRatio.array, 1]}
              count={count}
            />
            <bufferAttribute attach="attributes-aAngle" 
              args={[attributes.aAngle.array, 1]}
              count={count} />
            <bufferAttribute attach="attributes-aRandomOffset" 
